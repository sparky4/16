...
#define TAIL_FUNCTIONKEYFUNCTIONS0EXE \
	...
	if(IN_KeyDown(sc_F7)){ ZC_ShowMV(&MVVAR, 0, 1);						IN_UserInput(1); } \
	if(IN_KeyDown(sc_F6)){ ZC_ShowMV(&MVVAR, 0, 0);						IN_UserInput(1); } \
...
#define FUNCTIONKEYSHOWMV \
	if(IN_KeyDown(1+1)){ gvar.video.sp=0; ZC_ShowMV(&MVVAR, 0, 0); } \
	if(IN_KeyDown(2+1)){ gvar.video.sp=1; ZC_ShowMV(&MVVAR, 0, 0); } \
	if(IN_KeyDown(3+1)){ gvar.video.sp=2; ZC_ShowMV(&MVVAR, 0, 1); } \
	if(IN_KeyDown(4+1)){ gvar.video.sp=3; ZC_ShowMV(&MVVAR, 0, 1); }

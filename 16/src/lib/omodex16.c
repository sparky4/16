...
void
omodexPanPage(opage_t *page, int dx, int dy) {
    page->dx = dx;
    page->dy = dy;
}
...
void
omodexClearRegion(opage_t *page, int x, int y, int w, int h, byte  color) {
...
void
omodexDrawBmp(opage_t *page, int x, int y, bitmap_t *bmp) {
    /* draw the region (the entire freakin bitmap) */
    omodexDrawBmpRegion(page, x, y, 0, 0, bmp->width, bmp->height, bmp);
}


void
omodexDrawBmpRegion(opage_t *page, int x, int y,
...
void
omodexDrawSprite(opage_t *page, int x, int y, bitmap_t *bmp) {
    /* draw the whole sprite */
    omodexDrawSpriteRegion(page, x, y, 0, 0, bmp->width, bmp->height, bmp);
}

void
omodexDrawSpriteRegion(opage_t *page, int x, int y,
		      int rx, int ry, int rw, int rh, bitmap_t *bmp) {
    word poffset = (word)page->data + y*(page->width/4) + x/4;
    byte *data = bmp->data;
    word bmpOffset = (word) data + ry * bmp->width + rx;
 ...
 */
void
omodexCopyPageRegion(opage_t *dest, opage_t *src,
		    word sx, word sy,
		    word dx, word dy,
...
/* fade and flash */
void
omodexFadeOn(word fade, byte *palette) {
    fadePalette(-fade, 64, 64/fade+1, palette);
}


void
omodexFadeOff(word fade, byte *palette) {
    fadePalette(fade, 0, 64/fade+1, palette);
}


void
omodexFlashOn(word fade, byte *palette) {
    fadePalette(fade, -64, 64/fade+1, palette);
}


void
omodexFlashOff(word fade, byte *palette) {
    fadePalette(-fade, 0, 64/fade+1, palette);
}
...
void
omodexLoadPalFile(byte *filename, byte **palette) {
    FILE *file;
    byte *ptr;
...
void
omodexSavePalFile(char *filename, byte *pal) {
//    unsigned int i;
    FILE *file;
...
omodexPalUpdate(byte *p) {
    ...
    omodexWaitBorder();	    /* waits one retrace -- less flicker */

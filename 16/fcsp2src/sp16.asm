...
a_dorow PROC NEAR
	sub	byte ptr ds:a_musiccnt,1;;;;
	jnc	@@0
	mov	byte ptr ds:a_musiccnt,8;;;;music_speed
	mov	cx,music_channels
	mov	di,OFFSET music_patterns
	xor	bx,bx
@@1__:	sub	byte ptr ds:a_chdelaycnt[bx],1;;;;
	jns	@@2
	mov	si,ds:[di]
	xor	ax,ax
	call	a_playnote
...
;======do timer simulation stuff
	setborder 28
	xor	cx,cx
	mov	byte ptr ds:scrollsubber,0;;;;
	xchg	cx,ds:framecounter
	jcxz	@@78
@@77:	push	cx
	add	byte ptr ds:scrollsubber,90;;;;;SCROLLSPEED
	call	doit70
	pop	cx
	loop	@@77
	setborder 26
...
doit70	PROC NEAR
;---add sinuses & udforce
	add	byte ptr ds:sinus1,70;;;;
	add	byte ptr ds:sinus2,177;;;;
	add	word ptr ds:udforced,3000;;;;
;---set wave1
	mov	bx,OFFSET wwave
	mov	ax,77
	call	addwcos
...
;---add more text to 3d scroller
	sub	byte ptr ds:textcnt,90;;;;SCROLLSPEED
	jnc	@@t1
